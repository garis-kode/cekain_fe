  ...
  <div class="flex flex-col min-h-full">
    ...
    <Form @submit="handleRegister" :validation-schema="schema">
      <div class="my-6">
        ...
        <div class="mb-3">
          <label for="passwordConfirm" class="block mb-2 text-sm font-semibold text-gray-900 dark:text-white">Password Confirmation</label>
          <Field name="passwordConfirm" type="password" id="passwordConfirm" class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="•••••••••" />
          <ErrorMessage name="passwordConfirm" class="text-red-500 text-sm" />
        </div> 
        <div class="flex items-start mb-3">
          <div class="flex items-center h-5">
            <input id="privacyPolicy" type="checkbox" value="" class="w-4 h-4 border border-gray-300 rounded bg-gray-50 focus:ring-3 focus:ring-blue-300 dark:bg-gray-700 dark:border-gray-600 dark:focus:ring-blue-600 dark:ring-offset-gray-800" required />
          </div>
          <label for="privacyPolicy" class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">I agree with the <NuxtLink to="/privacy-policy"  class="text-blue-600 hover:underline dark:text-blue-500">Privacy Policy</NuxtLink>.</label>
        </div>
      </div>
      <button type="submit"
        :disabled="isLoading"
      ...
      >
        ...
        <img v-else src="/assets/img/icon/google.svg" alt="">
        <span class="ml-2">{{ isLoadingGoogle ? 'Loading...' : 'Google' }}</span>
      </button>
    </form>
...
useHead({
  title: 'Cekain - Sign Up'
});

const { registerAPI, googleAuthAPI } = useAuthAPI();
...
const handleOnSuccess = (response) => {
  ...
  googleAuthAPI(response.access_token)
    .then(response => {
      if (response.success) {
    ...
    .catch(err => {
      error.value = err.message || 'TAn unexpected error occurred.';
    })
    .finally(() => {
...
const handleOnError = (errorResponse) => {
  error.value = errorResponse || 'An unexpected error occurred.';
};
